({
    ...
    yesResponse : function(component, event, helper) {
        ...
        if(capitalRequestRecordstatus != "Closed"){
        component.set("v.CapitalRequestRecord.Status__c", "Cancelled");
       component.find("cfrRec").saveRecord($A.getCallback(function(saveResult) {
        if (saveResult.state === "SUCCESS" || saveResult.state === "DRAFT") {
            alert("CFR cancelled successfully.");
            ...
            alert('Unknown problem, state: ' + saveResult.state + ', error: ' + 
             ...
             component.set("v.recordSaveError", 'CFR cannot be cancelled as it is closed.');
            
            alert("CFR cannot be cancelled as it is closed.");
        }
